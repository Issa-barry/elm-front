...
var AuthService = class _AuthService {
  ...
  extractUserFromPayload(payload) {
    ...
    const roles = this.mergeUniqueStrings(this.normalizeStringList(source?.roles), this.normalizeStringList(source?.role_names), this.normalizeStringList(payload?.roles), this.normalizeStringList(payload?.role_names));
    if (roles.length > 0) {
      user.roles = roles;
  ...
  */
  ...
  hasAnyPermission(requiredPermissions) {
    ...
    if (permissions.length === 0)
      return false;
    const normalizedCurrent = new Set(permissions.map((permission) => this.normalizePermission(permission)).filter((permission) => permission.length > 0));
  ...
  hasAnyRole(requiredRoles) {
    ...
    if (!user)
      return false;
    const currentRoles = new Set([...user.roles ?? [], ...user.role_names ?? []].map((roleItem) => this.normalizeRole(roleItem)).filter((roleItem) => roleItem.length > 0));
...
//# sourceMappingURL=chunk-JKMJDVN5.js.map
