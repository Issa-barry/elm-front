...
var AppLayout = class _AppLayout {
  ...
  updateAccueilBodyClass(url) {
    ...
    const isAccueil = path === "/" || path === "" || /^\/dashboard(?:$|[-/])/.test(path);
    if (isAccueil) {
...
function clearSwCacheAndBoot() {
  return __async(this, null, function* () {
    if ("serviceWorker" in navigator) {
      const registrations = yield navigator.serviceWorker.getRegistrations();
      yield Promise.all(registrations.map((r) => r.unregister()));
    }
    if ("caches" in window) {
      const keys = yield caches.keys();
      yield Promise.all(keys.map((k) => caches.delete(k)));
    }
  });
}
clearSwCacheAndBoot().then(() => bootstrapApplication(AppComponent, appConfig)).catch(showBootstrapError);
