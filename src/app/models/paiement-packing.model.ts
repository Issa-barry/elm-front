...
// Types pour le mode de paiement
...
export class Versement {
  ...
  paiement_packing?: PaiementPacking;

  constructor(data: Partial<Versement> = {}) {
    this.id = data.id ?? 0;
    this.reference = data.reference ?? '';
    this.paiement_packing_id = data.paiement_packing_id ?? 0;
    ...
    this.paiement_packing = data.paiement_packing;
  }
}
...
export interface VersementStoreResponse {
  versement: Versement;
  paiement: PaiementPacking;
}

// ========================= PaiementPacking =========================

export class PaiementPacking {
  id: number;
  reference: string;
  ...
  constructor(data: Partial<PaiementPacking> = {}) {
    this.id = data.id ?? 0;
    this.reference = data.reference ?? '';
...
export interface StorePaiementPackingDto {
  prestataire_id: number;
  periode_debut: string;
  periode_fin: string;
  date_paiement: string;
...
// Prévisualisation paiement (retour de /preview)
export class PreviewPaiementPacking {
  prestataire_id: number;
  periode_debut: string;
  periode_fin: string;
  nb_packings: number;
  ...
  constructor(data: Partial<PreviewPaiementPacking> = {}) {
    this.prestataire_id = data.prestataire_id ?? 0;
    this.periode_debut = data.periode_debut ?? '';
    this.periode_fin = data.periode_fin ?? '';
    this.nb_packings = data.nb_packings ?? 0;
...
export interface PaiementPackingFilters {
  prestataire_id?: number;
  periode_debut?: string;
  periode_fin?: string;
  date_paiement_debut?: string;
  date_paiement_fin?: string;
  mode_paiement?: ModePaiement;
  search?: string;
  sort_by?: 'date_paiement' | 'created_at' | 'montant_total' | 'nb_packings';
  sort_order?: 'asc' | 'desc';
  per_page?: number;
}
